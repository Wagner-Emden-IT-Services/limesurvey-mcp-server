...
It is designed to inherit the GPL-licensed LimeSurvey fruity_twentythree theme.
 ...
 * Inherits the official fruity_twentythree Bootstrap 5 survey theme.
 */
:root {
...
async function validateZip(config: LimeSurveyConfig, fileName: string): Promise<JsonValue> {
  ...
  if (configEntry) {
    ...
    try {
      ...
      if (parent !== "fruity_twentythree") errors.push("Manifest must extend fruity_twentythree.");
...
export function registerThemeTools(server: McpServer, client: LimeSurveyClient, config: LimeSurveyConfig): void {
  ...
  server.registerTool(
    "limesurvey_generate_survey_theme",
    {
      ...
      description: "Create a validated GPL-2.0-or-later ZIP theme that inherits the official Bootstrap 5 fruity_twentythree theme for LimeSurvey 7 or 6.",
    ...
    async (input) => run(config, "generate_survey_theme", true, async () => {
      ...
      return {
        ...
        parent_theme: "fruity_twentythree",
        compatibility: [`${input.target_limesurvey_major}.0`],
        tested_reference: "LimeSurvey CE 7.0.5 / fruity_twentythree manifest checked 2026-07-22",
        contrast: {
  ...
  server.registerTool(
    ...
    async (input) => run(config, "get_theme_publication_guide", false, async () => {
      ...
      const steps: JsonValue[] = [
        ...
        `Confirm that the parent theme fruity_twentythree is installed. The imported theme should appear as ${input.theme_name} and extend fruity_twentythree.`,
      ...
      return {
        ...
        stop_conditions: [
          ...
          "fruity_twentythree is unavailable.",
